...
// assembly, and the render → post-process → write pipeline. Tools own correctness; this is it.
...
import { filename, KINDS, type Kind, PRESETS } from '../presets.js';
...
export const kindSchema = z
  ...
  .describe(
    'Purpose preset. icon: 1:1 flash → 512 square. token: 1:1 flash, top-down full body on a ' +
      'chroma plate (framing and plate text are appended for you). portrait: 3:4 pro at 2K. ' +
      'illustration: 16:9 pro at 4K → 2560×1600 (16:10 crop). Pro kinds need confirmPro.'
  );
...
/** Render one image through the API, post-process it, write it, meter it. */
export async function render(deps: ToolDeps, input: RenderInput): Promise<RenderOutput> {
  ...
  const file = path.join(deps.outputDir, filename(input.kind, input.slug, newId()));
